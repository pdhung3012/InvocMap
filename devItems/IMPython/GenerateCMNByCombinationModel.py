...
for idxFile in range(1,101):
    ...
    arrInputTokensPre = getLowerList(arrPre)
    vectorPre=modelPrefix.infer_vector(arrInputTokensPre)
    simsPre = modelPrefix.docvecs.most_similar([vectorPre], topn=200)
    arrInputTokensPost = getLowerList(arrPost)
    vectorPost = modelPostfix.infer_vector(arrInputTokensPost)
    simsPost = modelPostfix.docvecs.most_similar([vectorPost], topn=200)
    arrMethodOut=[]
    for index in range(0, len(simsPre)):
        key = int(simsPre[index][0])
        arrMethodOut.append(methodNames[key].strip())
    for index in range(0, len(simsPost)):
        key = int(simsPost[index][0])
        arrMethodOut.append(methodNames[key].strip())
